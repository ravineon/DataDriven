...
public class ReadExcelTestScenarios {
	...
	public ArrayList readFullXL(String filePath, String sheet_name, String release_num) {
		...
		try {
			...
			for(int i=0; i<rows; i++) {
				...
				for(int j=0; j<cols; j++) {
						...
						if (type == 0 ) {
							...
							((ArrayList)TestScenarios.get(i)).add(value);
						}
						type=-1;
			...
			for(int r=0; r<TestScenarios.size(); r++) {
			...
			System.out.println(newLine); */
				...
				for(int c=0; c <((ArrayList)TestScenarios.get(r)).size(); c++) {
					if( c == 0 || c == ReleaseCol) {
					String data = (String)((ArrayList)TestScenarios.get(r)).get(c);
			...
			/* Debug System.out.println(" In Loop");
				for(int r=0; r<TestScenarios.size(); r++){
						...
						System.out.println(newLine);
						for(int c=0; c <((ArrayList)TestScenarios.get(r)).size(); c++) {
							System.out.print( (String)((ArrayList)TestScenarios.get(r)).get(c) +"  ");  
						}
				} 
			*/
